<?php
	...
	if(!$dbc){ // there was an error connecting
		...
		$uid = $_POST["uid"];
		$score_to_add = $_POST["score_to_add"];
	
		//querying data based on information
		$query = mysqli_query($dbc,"UPDATE `Group Members` SET member_score = member_score + '$score_to_add' WHERE group_uid = '$group_uid' AND member_uid = '$uid'");
